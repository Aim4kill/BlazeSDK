...
public class ReplicatedGamePlayer : Tdf
{
    static readonly TdfMemberInfo[] __typeInfos = [
        ...
        new TdfMemberInfo("NetworkQosData", "mNetworkQosData", 0xBB1BF300, TdfType.Struct, 5, true), // NQOS
        new TdfMemberInfo("PlayerAttribs", "mPlayerAttribs", 0xC21D3400, TdfType.Map, 6, true), // PATT
        new TdfMemberInfo("PlayerId", "mPlayerId", 0xC2990000, TdfType.UInt32, 7, true), // PID
        new TdfMemberInfo("NetworkAddress", "mNetworkAddress", 0xC2E97400, TdfType.Union, 8, true), // PNET
        new TdfMemberInfo("SlotId", "mSlotId", 0xCE990000, TdfType.UInt8, 9, true), // SID
        new TdfMemberInfo("SlotType", "mSlotType", 0xCECBF400, TdfType.Enum, 10, true), // SLOT
        new TdfMemberInfo("PlayerState", "mPlayerState", 0xCF487400, TdfType.Enum, 11, true), // STAT
        new TdfMemberInfo("TeamId", "mTeamId", 0xD2586D00, TdfType.UInt16, 12, true), // TEAM
        new TdfMemberInfo("TeamIndex", "mTeamIndex", 0xD2993800, TdfType.UInt16, 13, true), // TIDX
        new TdfMemberInfo("JoinedGameTimestamp", "mJoinedGameTimestamp", 0xD29B6500, TdfType.Int64, 14, true), // TIME
        new TdfMemberInfo("PlayerSessionId", "mPlayerSessionId", 0xD6990000, TdfType.UInt32, 15, true), // UID
    ];
    ...
    private TdfStruct<Blaze2SDK.Blaze.Util.NetworkQosData?> _networkQosData = new(__typeInfos[5]);
    private TdfMap<string, string> _playerAttribs = new(__typeInfos[6]);
    private TdfUInt32 _playerId = new(__typeInfos[7]);
    private TdfUnion<Blaze2SDK.Blaze.NetworkAddress> _networkAddress = new(__typeInfos[8]);
    private TdfUInt8 _slotId = new(__typeInfos[9]);
    private TdfEnum<Blaze2SDK.Blaze.GameManager.SlotType> _slotType = new(__typeInfos[10]);
    private TdfEnum<Blaze2SDK.Blaze.GameManager.PlayerState> _playerState = new(__typeInfos[11]);
    private TdfUInt16 _teamId = new(__typeInfos[12]);
    private TdfUInt16 _teamIndex = new(__typeInfos[13]);
    private TdfInt64 _joinedGameTimestamp = new(__typeInfos[14]);
    private TdfUInt32 _playerSessionId = new(__typeInfos[15]);
    ...
    public Blaze2SDK.Blaze.GameManager.SlotType SlotType
    {
        get => _slotType.Value;
