...
public class PlaygroupInfo : Tdf
{
    static readonly TdfMemberInfo[] __typeInfos = [
        ...
        new TdfMemberInfo("OwnerBlazeId", "mOwnerBlazeId", 0xBF7BB200, TdfType.UInt32, 8, true), // OWNR
        new TdfMemberInfo("Id", "mId", 0xC27A6400, TdfType.UInt32, 9, true), // PGID
        new TdfMemberInfo("UniqueKey", "mUniqueKey", 0xD6B97900, TdfType.String, 10, true), // UKEY
        new TdfMemberInfo("UUID", "mUUID", 0xD75A6400, TdfType.String, 11, true), // UUID
        new TdfMemberInfo("VoipNetwork", "mVoipNetwork", 0xDAFA7000, TdfType.Enum, 12, true), // VOIP
        new TdfMemberInfo("XnetNonce", "mXnetNonce", 0xE2EBA300, TdfType.Blob, 13, true), // XNNC
        new TdfMemberInfo("XnetSession", "mXnetSession", 0xE3397300, TdfType.Blob, 14, true), // XSES
    ];
    private ITdfMember[] __members;

    private TdfMap<string, string> _playgroupAttributes = new(__typeInfos[0]);
    private TdfBool _enableVoIP = new(__typeInfos[1]);
    private TdfUnion<Blaze2SDK.Blaze.NetworkAddress> _hostNetworkAddress = new(__typeInfos[2]);
    private TdfUInt8 _hostSlotId = new(__typeInfos[3]);
    private TdfEnum<Blaze2SDK.Blaze.Playgroups.PlaygroupJoinability> _playgroupJoinability = new(__typeInfos[4]);
    private TdfUInt16 _maxMembers = new(__typeInfos[5]);
    private TdfString _name = new(__typeInfos[6]);
    private TdfEnum<Blaze2SDK.Blaze.GameNetworkTopology> _networkTopology = new(__typeInfos[7]);
    private TdfUInt32 _ownerBlazeId = new(__typeInfos[8]);
    private TdfUInt32 _id = new(__typeInfos[9]);
    private TdfString _uniqueKey = new(__typeInfos[10]);
    private TdfString _uUID = new(__typeInfos[11]);
    private TdfEnum<Blaze2SDK.Blaze.VoipTopology> _voipNetwork = new(__typeInfos[12]);
    private TdfBlob _xnetNonce = new(__typeInfos[13]);
    private TdfBlob _xnetSession = new(__typeInfos[14]);
    ...
    public Blaze2SDK.Blaze.NetworkAddress HostNetworkAddress
    {
