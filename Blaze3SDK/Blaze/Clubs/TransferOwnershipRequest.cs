using EATDF;
using EATDF.Members;
using EATDF.Types;

namespace Blaze3SDK.Blaze.Clubs;

public class TransferOwnershipRequest : Tdf
{
    static readonly TdfMemberInfo[] __typeInfos = [
        new TdfMemberInfo("UserId", "mUserId", 0x8ACA6400, TdfType.Int64, 0, true), // BLID
        new TdfMemberInfo("ClubId", "mClubId", 0x8ECA6400, TdfType.UInt32, 1, true), // CLID
        new TdfMemberInfo("ExOwnersNewStatus", "mExOwnersNewStatus", 0xBEFCEE00, TdfType.Enum, 2, true), // OOSN
    ];
    private ITdfMember[] __members;

    private TdfInt64 _userId = new(__typeInfos[0]);
    private TdfUInt32 _clubId = new(__typeInfos[1]);
    private TdfEnum<Blaze3SDK.Blaze.Clubs.MembershipStatus> _exOwnersNewStatus = new(__typeInfos[2]);

    public TransferOwnershipRequest()
    {
        __members = [
            _userId,
            _clubId,
            _exOwnersNewStatus,
        ];
    }

    public override Tdf CreateNew() => new TransferOwnershipRequest();
    public override ITdfMember[] GetMembers() => __members;
    public override TdfMemberInfo[] GetMemberInfos() => __typeInfos;
    public static TdfMemberInfo[] GetTdfMemberInfos() => __typeInfos;
    public override string GetClassName() => "TransferOwnershipRequest";
    public override string GetFullClassName() => "Blaze::Clubs::TransferOwnershipRequest";

    public long UserId
    {
        get => _userId.Value;
        set => _userId.Value = value;
    }

    public uint ClubId
    {
        get => _clubId.Value;
        set => _clubId.Value = value;
    }

    public Blaze3SDK.Blaze.Clubs.MembershipStatus ExOwnersNewStatus
    {
        get => _exOwnersNewStatus.Value;
        set => _exOwnersNewStatus.Value = value;
    }

}

