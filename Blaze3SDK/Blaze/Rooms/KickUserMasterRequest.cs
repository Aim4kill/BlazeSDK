using EATDF;
using EATDF.Members;
using EATDF.Types;

namespace Blaze3SDK.Blaze.Rooms;

public class KickUserMasterRequest : Tdf
{
    static readonly TdfMemberInfo[] __typeInfos = [
        new TdfMemberInfo("BanUser", "mBanUser", 0x8A1BB500, TdfType.Bool, 0, true), // BANU
        new TdfMemberInfo("UserId", "mUserId", 0xB62A6400, TdfType.Int64, 1, true), // MBID
        new TdfMemberInfo("RoomId", "mRoomId", 0xCADA6400, TdfType.UInt32, 2, true), // RMID
        new TdfMemberInfo("SessionId", "mSessionId", 0xCF3A6400, TdfType.UInt32, 3, true), // SSID
    ];
    private ITdfMember[] __members;

    private TdfBool _banUser = new(__typeInfos[0]);
    private TdfInt64 _userId = new(__typeInfos[1]);
    private TdfUInt32 _roomId = new(__typeInfos[2]);
    private TdfUInt32 _sessionId = new(__typeInfos[3]);

    public KickUserMasterRequest()
    {
        __members = [
            _banUser,
            _userId,
            _roomId,
            _sessionId,
        ];
    }

    public override Tdf CreateNew() => new KickUserMasterRequest();
    public override ITdfMember[] GetMembers() => __members;
    public override TdfMemberInfo[] GetMemberInfos() => __typeInfos;
    public static TdfMemberInfo[] GetTdfMemberInfos() => __typeInfos;
    public override string GetClassName() => "KickUserMasterRequest";
    public override string GetFullClassName() => "Blaze::Rooms::KickUserMasterRequest";

    public bool BanUser
    {
        get => _banUser.Value;
        set => _banUser.Value = value;
    }

    public long UserId
    {
        get => _userId.Value;
        set => _userId.Value = value;
    }

    public uint RoomId
    {
        get => _roomId.Value;
        set => _roomId.Value = value;
    }

    public uint SessionId
    {
        get => _sessionId.Value;
        set => _sessionId.Value = value;
    }

}

