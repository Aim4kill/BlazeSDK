...
public class ReplicatedGameData : Tdf
{
    static readonly TdfMemberInfo[] __typeInfos = [
        ...
        new TdfMemberInfo("GameReportingId", "mGameReportingId", 0x9F3A6400, TdfType.UInt64, 8, true), // GSID
        new TdfMemberInfo("GameState", "mGameState", 0x9F3D2100, TdfType.Enum, 9, true), // GSTA
        new TdfMemberInfo("GameTypeName", "mGameTypeName", 0x9F4E7000, TdfType.String, 10, true), // GTYP
        ...
        new TdfMemberInfo("ServerNotResetable", "mServerNotResetable", 0xBB297300, TdfType.Bool, 17, true), // NRES
        new TdfMemberInfo("NetworkTopology", "mNetworkTopology", 0xBB4BF000, TdfType.Enum, 18, true), // NTOP
        new TdfMemberInfo("PersistedGameId", "mPersistedGameId", 0xC27A6400, TdfType.String, 19, true), // PGID
        new TdfMemberInfo("PersistedGameIdSecret", "mPersistedGameIdSecret", 0xC27CF200, TdfType.Blob, 20, true), // PGSR
        new TdfMemberInfo("PlatformHostInfo", "mPlatformHostInfo", 0xC28CF400, TdfType.Struct, 21, true), // PHST
        new TdfMemberInfo("PresenceMode", "mPresenceMode", 0xC3297300, TdfType.Enum, 22, true), // PRES
        new TdfMemberInfo("PingSiteAlias", "mPingSiteAlias", 0xC3387300, TdfType.String, 23, true), // PSAS
        new TdfMemberInfo("QueueCapacity", "mQueueCapacity", 0xC6387000, TdfType.UInt16, 24, true), // QCAP
        new TdfMemberInfo("SharedSeed", "mSharedSeed", 0xCE596400, TdfType.UInt32, 25, true), // SEED
        new TdfMemberInfo("TeamCapacity", "mTeamCapacity", 0xD2387000, TdfType.UInt16, 26, true), // TCAP
        new TdfMemberInfo("TopologyHostInfo", "mTopologyHostInfo", 0xD28CF400, TdfType.Struct, 27, true), // THST
        new TdfMemberInfo("TeamIds", "mTeamIds", 0xD2993300, TdfType.List, 28, true), // TIDS
        new TdfMemberInfo("UUID", "mUUID", 0xD75A6400, TdfType.String, 29, true), // UUID
        new TdfMemberInfo("VoipNetwork", "mVoipNetwork", 0xDAFA7000, TdfType.Enum, 30, true), // VOIP
        new TdfMemberInfo("GameProtocolVersionString", "mGameProtocolVersionString", 0xDB3D3200, TdfType.String, 31, true), // VSTR
        new TdfMemberInfo("XnetNonce", "mXnetNonce", 0xE2EBA300, TdfType.Blob, 32, true), // XNNC
        new TdfMemberInfo("XnetSession", "mXnetSession", 0xE3397300, TdfType.Blob, 33, true), // XSES
    ];
    ...
    private TdfUInt64 _gameReportingId = new(__typeInfos[8]);
    private TdfEnum<Blaze3SDK.Blaze.GameManager.GameState> _gameState = new(__typeInfos[9]);
    private TdfString _gameTypeName = new(__typeInfos[10]);
    private TdfList<Blaze3SDK.Blaze.NetworkAddress> _hostNetworkAddressList = new(__typeInfos[11]);
    ...
    private TdfStruct<Blaze3SDK.Blaze.Util.NetworkQosData?> _networkQosData = new(__typeInfos[16]);
    private TdfBool _serverNotResetable = new(__typeInfos[17]);
    private TdfEnum<Blaze3SDK.Blaze.GameNetworkTopology> _networkTopology = new(__typeInfos[18]);
    private TdfString _persistedGameId = new(__typeInfos[19]);
    private TdfBlob _persistedGameIdSecret = new(__typeInfos[20]);
    private TdfStruct<Blaze3SDK.Blaze.GameManager.HostInfo?> _platformHostInfo = new(__typeInfos[21]);
    private TdfEnum<Blaze3SDK.Blaze.PresenceMode> _presenceMode = new(__typeInfos[22]);
    private TdfString _pingSiteAlias = new(__typeInfos[23]);
    private TdfUInt16 _queueCapacity = new(__typeInfos[24]);
    private TdfUInt32 _sharedSeed = new(__typeInfos[25]);
    private TdfUInt16 _teamCapacity = new(__typeInfos[26]);
    private TdfStruct<Blaze3SDK.Blaze.GameManager.HostInfo?> _topologyHostInfo = new(__typeInfos[27]);
    private TdfList<ushort> _teamIds = new(__typeInfos[28]);
    private TdfString _uUID = new(__typeInfos[29]);
    private TdfEnum<Blaze3SDK.Blaze.VoipTopology> _voipNetwork = new(__typeInfos[30]);
    private TdfString _gameProtocolVersionString = new(__typeInfos[31]);
    private TdfBlob _xnetNonce = new(__typeInfos[32]);
    private TdfBlob _xnetSession = new(__typeInfos[33]);
    ...
    public Blaze3SDK.Blaze.GameManager.HostInfo? PlatformHostInfo
    {
    ...
    public Blaze3SDK.Blaze.GameManager.HostInfo? TopologyHostInfo
    {
